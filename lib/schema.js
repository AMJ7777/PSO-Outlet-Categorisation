...
/** Turns a column label into a storage field key: lowercase, non-alnum runs
 *  collapsed to underscores, and prefixed if that leaves nothing letter-led. */
function slugifyField(label) {
  const s = String(label || '').trim().toLowerCase().replace(/[^a-z0-9]+/g, '_').replace(/^_+|_+$/g, '');
  return s && /^[a-z]/.test(s) ? s : ('c_' + s);
}

/**
 * Admin-only. Adds a brand new column to the schema for a data category the
 * app didn't originally track (e.g. "EV Charging"). New columns always land
 * as facility fields (alongside cards/shop/vibe/...) and start optional, so
 * existing sheets keep importing without it. The field key is derived from
 * the label and de-duplicated if it collides. Throws with a user-facing
 * message on a blank or already-used column name.
 */
function addColumn(def) {
  const label = String((def && def.column) || '').trim();
  if (!label) throw new Error('Column name cannot be blank.');
  if (COLUMNS.some(function (c) { return normalizeHeader(c.column) === normalizeHeader(label); })) {
    throw new Error('Column name "' + label + '" already exists.');
  }

  let field = slugifyField(label);
  if (!field) throw new Error('Column name must contain at least one letter or number.');
  let unique = field, n = 2;
  while (COLUMNS.some(function (c) { return c.field === unique; })) { unique = field + '_' + n; n++; }
  field = unique;

  const type = ['string', 'number', 'bool'].indexOf(def && def.type) !== -1 ? def.type : 'string';
  const col = { column: label, field: field, type: type, facility: true, required: false };
  COLUMNS.push(col);
  reindex();
  return col;
}
